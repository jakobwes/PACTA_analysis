...
for(fin_data_timestamp in 1:2){#1:2
  ...
  data <- data %>% mutate(investor_name = "Meta Investor", portfolio_name = "Meta Portfolio")

  write_portfolio_csv(data, meta_output_dir, portfolio_name_ref_all = paste0(project_prefix, "_meta"))
  write_config_file(meta_output_dir, holdings_date = timestamps[fin_data_timestamp], given_investor_name = "Meta Investor", given_portfolio_name = "Meta Portfolio", portfolio_name_ref_all = paste0(project_prefix, "_meta"))
  ...
  for (user_id in all_user_ids) {
    ...
    write_config_file(user_id_output_dir, holdings_date = timestamps[fin_data_timestamp], given_investor_name = as.character(investor_name), given_portfolio_name = as.character(investor_name), peer_group = peer_group, portfolio_name_ref_all = paste0(project_prefix, "_user_", user_id))
    write_portfolio_csv(user_data, user_id_output_dir, portfolio_name_ref_all = paste0(project_prefix, "_user_", user_id))
  ...
  dir_copy(file.path("working_dir", "10_Parameter_File"), file.path(meta_output_dir, "10_Parameter_File"), overwrite = TRUE)
  dir_copy(file.path("working_dir", "20_Raw_Inputs"), file.path(meta_output_dir, "20_Raw_Inputs"), overwrite = TRUE)
  dir_copy(file.path("working_dir", "30_Processed_Inputs", portfolio_name_ref_all), file.path(meta_output_dir, "30_Processed_Inputs"), overwrite = TRUE)

  # webtool script 2
  dir.create(file.path(meta_output_dir, "40_Results"))
  ...
  for (user_id in all_user_ids) {
    ...
    dir_copy(file.path("working_dir", "10_Parameter_File"), file.path(user_id_output_dir, "10_Parameter_File"), overwrite = TRUE)
    dir_copy(file.path("working_dir", "20_Raw_Inputs"), file.path(user_id_output_dir, "20_Raw_Inputs"), overwrite = TRUE)
    dir_copy(file.path("working_dir", "30_Processed_Inputs", portfolio_name_ref_all), file.path(user_id_output_dir, "30_Processed_Inputs"), overwrite = TRUE)

    # web tool script 2
    dir.create(file.path(user_id_output_dir, "40_Results"))
